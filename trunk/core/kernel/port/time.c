...
void disarm_timer (ktimer_t *ktimer) {
  unsigned long flags;
...
duration_t sleep_for (duration_t d) {
  ...
  if (ktimer->timer_value > ctime) {
    disarm_timer (ktimer);
    return (ktimer->timer_value - ctime);
  }
  return 0;
}
...
asmlinkage int timer_settime_sys (timer_t timerid, int flags, 
  ...
  if (timer_array [timerid].flags & TIMER_ARMED) {
    ...
    disarm_timer (&timer_array [timerid]);
  } else {
    if (ovalue)
      memset (ovalue, 0, sizeof (struct itimerspec));
  }
