...
rtcgettime(RTCTime *time){
	time->usec = (CTC * SEC2USECS) / (2*RTCFREQ);
	time->sec = SEC;
	time->min = MIN;
	time->hour = HOUR;
	time->mday = DOM;
...
sec2rtc(ulong e, RTCTime *rtc){
	...
	// generate usec
	rtc->usec = (CTC * SEC2USECS) / (2*RTCFREQ);
...
static duration_t
rtcread_nsec(void){
	return rtcsecs * NSECS_PER_SEC + CTC * (NSECS_PER_SEC)/(hwtime_t)(2*RTCFREQ);
}

static hwtime_t
rtchwtime2dur(hwtime_t hw){
	return hw * (NSECS_PER_SEC/RTCFREQ);
}
